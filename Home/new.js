...
function generateNewArrival(){
    let cardscontainer=document.getElementById("newArrivalpage");
    return (cardscontainer.innerHTML=allBooks.slice(-10).map(function(x){
        return `
...
function generatenewModal(id){
    ...
    if (bookFind){
        let modal= document.getElementById("modalnewgenerateBody");
         modal.innerHTML = `
